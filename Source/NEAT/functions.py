...
class Functions(NEATConfigBase):
    ...
    def compare_genome_compatibility(self, _genome_one, _genome_two):
        """
        ...
        highest_num_genes = 1 if highest_num_genes < self.kGene_threshold else highest_num_genes

        num_excess_genes, num_disjoint_genes = self._calc_excess_disjoint_genes(_genome_one, _genome_two)
        avg_wt_diff = self._calc_avg_wt_difference(_genome_one, _genome_two)

        return self.kCompat_threshold > (self.kCoeff_excess * (num_excess_genes / highest_num_genes) +
                                         self.kCoeff_disjoint * (num_disjoint_genes / highest_num_genes) +
                                         self.kCoeff_wt_diff * avg_wt_diff)
        ...
        *Note this function may append to the list passed as all_species
        ...
        for individual in _individuals:
            ...
            for single_species in all_species:
                if self.compare_genome_compatibility(single_species.get_representative_genome(), individual.genome):
                    single_species.add_member(individual)
                    found_species_match = True
                    break
            if not found_species_match:
                all_species.append(Species(_current_generation, individual, config=self.config))
